...
export default function FundingJourney({companyId}) {
            ...
            try {
                ...
                if (result?.data?.[0]?.funding?.length > 0) {
                    ...
                    if (result?.data?.length === 0) {
                        setError(`Startup with ID ${companyId} not found or has no funding information.`);
                    }
                }
            } catch (err) {
    ...
    const processFundingData = (apiFundingRounds) => {
        try {
            ...
            apiFundingRounds.forEach((roundData, index) => {
                ...
                if (Array.isArray(roundData.investors) && roundData.investors.length > 0) {
                ...
                const source = roundInvestorNames.length > 0
    ...
    const SectionHeader = () => (
        ...
        >
            <div
                className="flex flex-col md:flex-row items-center justify-between md:justify-start mb-6 space-y-2 md:space-y-0 md:space-x-3 w-full">
                <div
        ...
        <span className="bg-clip-text text-transparent bg-gradient-to-r from-orange-500 to-red-600 ">
          Funding Journey
                ...
                <div
                    className="block md:hidden w-1/4 h-1.5 bg-gradient-to-r from-orange-500 to-red-500 rounded-full"></div>
            </div>
        </div>
    ...
    const FundingRound = ({round, isVisible}) => (
        <div
            className={`relative pl-10 transform transition-all duration-700 ${round.delay} ${
                isVisible ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0"
            } group`}
        >
            <div
                className="absolute left-3.5 top-0 bottom-0 w-0.5 bg-gradient-to-b from-orange-200 via-orange-300 to-orange-400 rounded-full mb-8"></div>
            <div
            ...
            <div
                className="bg-white p-4 sm:p-5 rounded-xl transition-all duration-500">
     ...
     */
    const InvestorCard = ({investor, index, isHovered, onMouseEnter, onMouseLeave, isVisible}) => (
        <div
            key={`${investor.name}-${index}`}
            className={`transform transition-all duration-700 delay-${(index % 5) * 100 + 300} ${
                isVisible ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0"
            }`}
     ...
     */
    const InvestorsList = ({investors, hoveredInvestor, setHoveredInvestor, isVisible}) => (
        <>
            <h3 className="text-xl sm:text-2xl font-bold text-gray-800 mb-5 sm:mb-6 flex items-center">
                <div className="w-1 h-6 sm:h-7 bg-orange-500 rounded-full mr-3"></div>
                ...
                {!loading && !error && fundingRounds.length > 0 && (
                    <div className="grid lg:grid-cols-2 gap-8 md:gap-12 w-full px-[69px] mx-auto">
                        ...
                        >
                            ...
                            >
                                <div
                                    className="bg-gradient-to-r from-orange-500 to-orange-600 p-6 rounded-2xl shadow-xl text-white transform hover:scale-105 transition-all duration-300">
