...
const FeatureCard = ({ type, source, title, date, link }) => {
...
export default function PressFeaturesSection({ companyId }) {
  ...
  for (let i = 0; i < allItems.length; i += 6) {
    chunkedItems.push(allItems.slice(i, i + 6));
  }

  // --- Carousel Settings ---
  const sliderSettings = {
    dots: true,
    infinite: allItems.length > 3, // Only loop if there are more items than slides to show
    speed: 500,
